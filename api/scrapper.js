...
export const scrapePageData = async (url) => {
  ...
  const initialData = await page.evaluate(async () => {
      ...
      document
        ...
        .forEach((element) => {
          ...
          const locationElement = element.querySelector(
            '.styles-module_content__location-container__pRGhf > p'
          );
    ...
    let nextButton = document.querySelector(
      '.styles_pagination__e_iw6 > button:last-child'
    );
  ...
  for (let item of initialData) {
    if (item.articleURL !== 'No URL') {
      await page.goto(item.articleURL, {
      ...
      const dld_permit_number = await page.evaluate(() => {
        return (
          document.querySelector(
            '.styles_desktop_container__kcSkl .styles_desktop_content__Z_YaU .styles_desktop_link___qw0V .styles_desktop_value__mxst1'
          )?.textContent || 'No DLD Permit Number'
        );
      });
      finalData.push({
